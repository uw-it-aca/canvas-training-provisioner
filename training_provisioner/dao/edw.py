...
class EDWConnection:
    ...
    Supports mock data for local development environments. See README for details.
    """
    ...
    def __init__(self):
        ...
        if not all([self.host, self.user, self.password]):
            raise ValueError("EDW connection parameters (EDW_HOST, EDW_USER, EDW_PASS) must be configured in settings")

    def _get_connection_string(self):
        """Build the SQLAlchemy connection string for EDW."""
        return f"mssql+pymssql://{quote_plus(self.user)}:{quote_plus(self.password)}@{self.host}"

    def execute_query(self, query):
        """
        Execute a SQL query against the EDW and return results as a Pandas DataFrame.
        ...
        Raises:
            DataAccessException: If there's an error connecting or executing the query
        """
        if not query or not query.strip():
            raise ValueError("Query cannot be empty")

        # In localdev environment, return mock data instead of executing real queries
        if self.use_mock_data:
        ...
        try:
            ...
            logger.info(f"\tData read from {self.host}: {data.shape} (rows, columns)")
            ...
            raise DataAccessException(f"Failed to execute query against EDW: {e}")
    ...
    def _get_mock_data(self, query):
        """
        Return mock data for localdev environment based on calling function name.
        ...
        caller_frame = inspect.currentframe().f_back.f_back.f_back  # Skip execute_query frame
        caller_function = caller_frame.f_code.co_name if caller_frame else 'unknown'

        logger.debug(f"EDW mock data requested by function: {caller_function}")
        ...
        if not os.path.exists(mock_file):
            ...
            logger.warning(f"No mock data file found for function '{caller_function}', using default")
        ...
        try:
            ...
            logger.info(f"Loaded mock data from {os.path.basename(mock_file)}: {mock_data.get('description', 'No description')}")

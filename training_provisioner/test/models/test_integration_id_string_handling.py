...
class IntegrationIdStringHandlingTest(TrainingCourseTestCase):
        ...
        for enrollment in Enrollment.objects.all():
            with self.subTest(enrollment_id=enrollment.id):
                self.assertIsInstance(
                    ...
                    f"integration_id should be string, got {type(enrollment.integration_id)}: {enrollment.integration_id}"
                )
    ...
    def test_django_converts_int_to_string_for_queries(self):
        """Test that Django automatically converts int to string for CharField queries"""
        test_id_int = 5432101
        test_id_str = "5432101"
    ...
    def test_enrolled_studentnos_query_returns_strings(self):
        ...
        for student_no in enrolled_studentnos:
            with self.subTest(student_no=student_no):
                self.assertIsInstance(
                    student_no, str,
                    f"values_list should return strings, got {type(student_no)}: {student_no}"
                )

    @patch('training_provisioner.models.'
           'training_course.TrainingCourse.get_course_membership')
    def test_membership_candidates_should_be_converted_to_strings(self, mock_membership):
        """Test that membership candidates should be converted to strings to avoid issues"""
        # Mock with mixed types to test conversion
        mock_membership.return_value = ["5432101", "5432102", 5432103, 5432104]
    ...
    def test_course_id_for_member_with_string_integration_id(self):
        """Test that get_course_id_for_member works with string integration_id"""
        test_id = "5432101"
        ...
        self.assertTrue(course_id.startswith(self.training_course.course_id_prefix))

    def test_course_id_for_member_with_int_integration_id(self):
        """Test that get_course_id_for_member works with int integration_id"""
        test_id_int = 5432101
        test_id_str = "5432101"
        
        # Both should work due to _hash() converting to int
        course_id_from_int = self.training_course.get_course_id_for_member(test_id_int)
        course_id_from_str = self.training_course.get_course_id_for_member(test_id_str)
        
        # Should return the same course_id
        self.assertEqual(course_id_from_int, course_id_from_str)

    def test_create_enrollment_with_int_integration_id_converts_to_string(self):
        """Test that creating enrollment with int integration_id gets stored as string"""
    ...
    def test_real_world_scenario_enrolled_vs_candidates_mismatch(self):
        """Test the real-world scenario where enrolled students and candidates have type mismatches"""
        ...
        for candidate in mixed_candidates:
            # Without string conversion, discard silently fails for int candidates
            ...
            if isinstance(candidate, int):
                ...
                if candidate_str in enrolled_studentnos:
                    # The discard should have removed it, but didn't due to type mismatch
                    self.assertIn(candidate_str, enrolled_copy, 
                                f"discard() failed to remove {candidate} due to type mismatch")
            else:
                # String candidates work correctly
                if candidate in enrolled_studentnos:
                    self.assertNotIn(candidate, enrolled_copy,
    ...
    def test_solution_always_convert_candidates_to_strings(self):
        ...
        for candidate in mixed_candidates:
            ...
            if candidate_str in enrolled_studentnos:
                ...
                self.assertNotIn(candidate_str, enrolled_copy,
                               f"discard() correctly removed {candidate_str} after string conversion")

    def test_hash_function_consistency_with_string_and_int(self):
        """Test that _hash() function works consistently with string and int inputs"""
        test_id_str = "5432101"
        test_id_int = 5432101
    ...
    def test_course_index_for_member_consistency(self):
        ...
        index_from_str = self.training_course._course_index_for_member(test_id_str)
        index_from_int = self.training_course._course_index_for_member(test_id_int)
    ...
    def test_filter_candidates_by_course_type_string_consistency(self):
        """Test that _filter_candidates_by_course_type handles string IDs correctly"""
